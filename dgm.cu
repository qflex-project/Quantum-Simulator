...
void PCpuExecution1(std::complex <float> *state, PT **pts, int qubits, long n_threads, int coales, int region, int it){
	...
	while (pts[i] != NULL){
		...
		end = i;													//Executa até o operador na posiÃ§ao 'i' (exclusive) nesta iteração
		...
		#pragma omp parallel
		{
			...
			while (reg_id != -1){		
				...
				//Define a prÃ³xima região (reg_id) da thread
				#pragma omp critical (teste)
				{
					reg_id = ext_reg_id;
					ext_reg_id = (ext_reg_id + reg_mask + 1) & ~reg_mask;
...
void DGM::HybridExecution(PT **pts){
	...
	while (pts[i] != NULL){
		...
		global_reg_count = (1 << (qubits - global_region)) + 1; 				//Némero de regiões	- +1 para a condição de parada incluir todos
